...
from integrated_led_manipulation import PiIntegratedLED
...
class LEDController:
    ...
    def _led_loop(self):
        self.led.set_trigger("none")
        while self.running:
            if self.state == "processing":
                self.led.on()
                time.sleep(0.1)
                if not self.running or self.state != "processing":
                    continue
                self.led.off()
                time.sleep(0.1)
                ...
                self.led.on()
                time.sleep(0.5)
                if not self.running or self.state != "transmitting":
                    continue
                self.led.off()
                time.sleep(0.5)
            else:
    ...
    def stop(self):
        ...
        if self.thread.is_alive():
            self.thread.join()
        self.led.set_trigger(self.led.original_trigger)
        if self.led.original_trigger == "none":
            self.led.on()
...
def run(args):
    ...
    if args.serial_port:
        try:
            ...
            led_ctrl.set_state("transmitting")
