#!/usr/bin/env python3
"""
CanSat Pipeline - Orchestration and XBee Transmission (External LED)
...
Includes external LED indicator for processing and transmission phases.
"""
...
# Impor GPIOZero for external LED
try:
...
def load_calibration(pipeline, calib_path: str):
    ...
    if not path.exists():
        log.warning(
            "Calibration file not found: %s (proceeding without calibration)",
            calib_path,
        )
        return False
    ...
    try:
        ...
        log.warning(
            "OpenCV not available in Python; cannot compute remap from intrinsics"
        )
        return False
...
def run(args):
    ...
    if LED:
        try:
            ext_led = LED(args.led_pin)
            log.info("External LED initialized on GPIO %d", args.led_pin)
        except Exception as e:
            log.error("Failed to initialize external LED on GPIO %d: %s", args.led_pin, e)
    else:
        log.warning("gpiozero not installed. External LED will not work. Install with: pip install gpiozero")
    ...
    # Run acquisition
    if ext_led:
        ext_led.blink(on_time=0.1, off_time=0.1) # Parpadeo rápido durante el procesamiento
    images = pipeline.run_acquisition()
    if ext_led:
        ext_led.off() # Detiene el parpadeo
    ...
    if args.serial_port:
        try:
            ...
            log.info(
                "XBee serial opened: %s @ %d baud", args.serial_port, args.baudrate
            )

            if ext_led:
                ext_led.blink(on_time=0.5, off_time=0.5) # Parpadeo de 1s de periodo durante la transmisión
...
def main():
    log.info("Waiting 15 seconds before execution...")
    time.sleep(15)

    parser = argparse.ArgumentParser(
        description="CanSat Image Pipeline - Capture, Process, Transmit (External LED)"
    )
    ...
    parser.add_argument(
        "--num-captures", type=int, default=3, help="Number of captures"
    )
    parser.add_argument(
        "--interval", type=int, default=3, help="Seconds between captures"
    )
    parser.add_argument(
        "--calibration",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=17,
        help="GPIO pin for the external LED indicator (Default: 17)",
    )
